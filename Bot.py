...
for g in fvgs:
if g["dir"] == d and rows[i]["l"] <= g["top"] + .1 * atrV and rows[i]["l"] >= g["bot"] - .1 * atrV:
return 1
if ob and ob["dir"] == d and rows[i]["l"] <= ob["top"] + .1 * atrV and rows[i]["l"] >= ob["bot"] - .1 * atrV:
return 1
return 0
...
trig = -2
elif sC["c"] > sC["o"] and sC["c"] >= sC["l"] + .5 * cr:
trig = 1
elif sC["c"] < sC["o"] and sC["c"] <= sC["h"] - .5 * cr:
trig = -1
...
dir_, pb = -1, "SWEEP"
if not dir_ and (regime == "TREND" or er > .25):
...
dir_, pb = -1, "BOS"
if not dir_ and regime != "TREND" and bullT and rangePos <= .4:
dir_, pb = 1, "FADE"
if not dir_ and regime != "TREND" and bearT and rangePos >= .6:
dir_, pb = -1, "FADE"
...
w += 1 if rangePos <= .3 else (-1 if rangePos >= .7 else 0)
if not ISN(vwap):
w += .5 if px > vwap else -.5
if w > .5:
dir_, pb = 1, "HYBRID"
elif w < -.5:
dir_, pb = -1, "HYBRID"
...
be = px + .35 * ds if dir_ > 0 else px - .35 * ds
m = 30
...
if dir_:
mu = .6 * dir_ * mean8
mu += .08 * sd if (cvdS > 0) == (dir_ > 0) else -.04 * sd
mu += .08 * sd if volX else 0
ht = resampleF(rows, 4)
hc = [x["c"] for x in ht]
he = emaF(hc, 20)
htUp = hc[-1] > he[-1]
mu += .10 * sd if ((htUp and dir_ == 1) or (not htUp and dir_ == -1)) else -.12 * sd
fit = (regime == "TREND" and pb in ("PULLBACK", "BOS", "CONT")) or 
 (regime == "RANGE" and pb in ("FADE", "SWEEP", "VWAP")) or pb in ("SWEEP", "HYBRID")
mu += .06 * sd if fit else -.08 * sd
mu = max(-.25 * sd, min(.45 * sd, mu))
spread = .0006 * px
...
trs.append(max(rows[i]["h"] - rows[i]["l"], abs(rows[i]["h"] - rows[i - 1]["c"]), abs(rows[i]["l"] - rows[i - 1]["c"])))
...
used = len(S["active"])
notify(card_signal(key.replace("USDT", ""), s["dir"], s["px"], s["tp"], s["sl"],
